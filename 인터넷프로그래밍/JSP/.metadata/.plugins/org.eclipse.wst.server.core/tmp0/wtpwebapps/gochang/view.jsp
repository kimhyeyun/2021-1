		...
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 2px solid royalblue;">
				...
				<tbody>
					...
					<tr>
						<td>내용</td>
						<td colspan="2" style="min-height:200px; text-align:left;"><%= board.getBdContent().replaceAll(" ", "&nbsp;").replaceAll("\n","<br>") %></td>
					</tr>
				</tbody>
			</table>
			<a href="community.jsp" class="btn btn-primary">목록</a>
			<%
				if((userID != null && userID.equals(board.getUserID())) || (userID != null && userID.equals("rhckdrnr123"))){
			%>
				<a href="update?bdID=<%= bdID %>" class="btn btn-primary">수정</a>
				<a onclick="return confirm('삭제하시겠습니다?')" href="deleteAction.jsp?bdID=<%= bdID %>" class="btn btn-primary">삭제</a>
			<%
				}
			%>
