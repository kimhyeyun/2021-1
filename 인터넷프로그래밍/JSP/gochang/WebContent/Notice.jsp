		...
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 2px solid royalblue">
				...
				<tbody>
					...
					%>
					<tr>
						<td><%= list.get(i).getNtID() %></td>
						<td><a href="viewNotice.jsp?ntID=<%= list.get(i).getNtID() %>"><%= list.get(i).getNtTitle() %></a></td>
						<td><%= list.get(i).getUserID() %></td>
						<td><%= list.get(i).getNtDate() %></td>
					</tr>
					<%		
						}
					%>
			...
			%>
			<%
				if(userID != null && userID.equals("rhckdrnr123")) { // 관리자아이디 : rhckdrnr123 -> 관리자만 공지사항작성가능
			%>
				<a href="Not_write.jsp" class="btn btn-primary pull-right">글쓰기</a>
			<%
				}
			%>
		</div>
