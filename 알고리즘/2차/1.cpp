...
class Navigation{
  public:
    ...
    int Idx[1000000];                               //지역 구조체와 연결하기위한 배열
    int dist[1000000];                              //최단 거리 저장
    bool visited[1000000];                          //방문 여부
    ...
    void dijkstra(int start, int destination){
        ...
        while(!pq.empty()){
            ...
            //이어진 도로 탐색
            for(int i=0;i<adj[minTo].size();i++){
...
int main(){
    ...
    Navigation *navi = new Navigation(n);
