...
int main(){
	...
	// 지역 정보를 지역 번호 순으로 정렬
	sort(location + 1, location + n + 1,comp);
	...
	for (int i = 1; i <= m; i++) {
		...
		navi[locIdx[start]].push_back(make_pair(locIdx[end],weight ));
		navi[locIdx[end]].push_back(make_pair(locIdx[start],weight ));
	}
...
void dijkstra(int s, int d) {
	...
	for (int i = 0; i < navi[s].size(); i++) {
		if (!location[s].flood && !location[navi[s][i].first].flood) {
			...
			fringe.push(make_pair( -navi[s][i].second, make_pair( -navi[s][i].first, s)));
		}
	}
	...
	while (fringe.size()) {
		...
		if (!inTree[minIdx]) {
            ...
            edge e = {minIdx ,minFrom, minWeight};
			tree.push_back(e);		// tree set에 도착점을 추가 
		...
		for (int i = 0; i < navi[minIdx].size(); i++) {
			if (!location[minIdx].flood && !location[navi[minIdx][i].first].flood) {
				...
				if (!inTree[navi[minIdx][i].first]) {
					...
					fringe.push(make_pair(-(navi[minIdx][i].second + dist[minIdx]), make_pair(-navi[minIdx][i].first, minIdx )));
				}
			}
		}
